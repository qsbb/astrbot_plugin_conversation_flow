...
- 支持核接管「一键读取当前配置 / 一键固化到插件」：一键读取/固化：snapshot 暴露 native_value；固化写入 data_dir/config.json（先备份 native-backup-<时间戳>.json、原子落盘、失败回滚、运行时同步）。
## 0.12.9 - 2026-09-16

### 变更

- 取消「小尾巴保护」：像「这次终于把所有自动化测试都顺利跑通了！好耶。」这类**尾句**（以句末标点收尾的情绪/回应）现在会按"一句话一条"独立成条，不再被并回上一条。
- 其余规则不变：单换行只对极短行（≤12 字）例外、冒号不切、关联词不强制合、语气收尾+转折另起一条；主链用空行表达的分条始终优先。
## 0.12.8 - 2026-09-16
...
- 设置中心布尔字段改用 series.ui 统一滑块开关，移除页面写死的勾选框尺寸，避免压扁共享控件。
...
- 新增 Plugin Page「设置中心」：由 `_conf_schema.json` 驱动渲染全部可编辑配置（分组折叠、修改高亮、未保存计数、撤销、保存后立即生效并持久化），没有核时也能在页面内完成全部配置修改；核覆盖层接管的项标注「核已覆盖」。
- 恢复 `pages/manager` 独立 Plugin Page（对话流状态、能力开关与配置摘要）：核 WebUI 面板继续作为增强入口，两者并存；核缺失或接管关闭时页面照常可用，符合规范 §9「standalone 优先」。

## 0.12.4 - 2026-09-14

### 变更

- 插话合并改为状态驱动（不再按"间隔几秒"判定）：她还在思考（未产出）时用户补充的消息一律并入当前轮，只回一条；她已经开始输出（分段投递中）时不再撤回旧回复，改为排队下一轮并注入"接着回应、不要重复"的衔接提示。
- 新增合并护栏：同一轮连续取消重跑最多 3 次、思考总时长超过 45 秒后退化为排队，避免用户连续补话导致回复被饿死。

## 0.12.3 - 2026-09-14

### 变更

- 核 WebUI「对话流状态」面板补齐 4 个比率与 4 项配置摘要，与独立 Page 展示口径一致。
- `pages/manager` 收敛为一句话引导页，功能入口统一指向核 WebUI「对话与消息」。
...
- 首屏关键比率与统计骨架顺序保持一致，消除加载完成后的整块重排。
- 移动端所有指标组统一为两列，避免普通组塌成单列把页面拉长。
- 判定策略/插话模式/插话作用域等枚举值改中文显示（指令注入、运行中插话归属、@Bot 等）。
- stale 提示与主体同宽，并取消重复的 toast 提示。

## 0.12.1 - 2026-09-13

### 变更

- 优化言页面信息层级和加载骨架；补齐共享 UI 布局组件。

## 0.12.0 - 2026-09-13

### 新增

- 全面接入 `series.ui@1.0` Glass Aurora 组件库，统一统计卡、状态条、表单和交互反馈，保留对话流专属信息结构。

### 既有未发布内容

## 0.11.0 - 2026-09-12

- 统一接管字段扩展到动态/近期/群聊上下文、读空气、追问保护、情绪与上下文预算等低风险运行开关。

## 0.10.0 - 2026-09-12

### 新增

- 新增统一上下文预算：P0 身份授权、P1 当前轮、P2 人格、P3 关系情绪、P4 情节记忆、P5 环境、P6 语义知识、P7 语音风格分层统计；默认 shadow 只诊断，显式严格模式才裁剪。
- 接入 `series.module@1.0` 与 `series.webui@2.0`，增加独立状态页和核托管状态面板；分段裁判、静默预判、合并重写统一使用 `fast` 模型角色。
- 上下文预算的启用、软上限、硬上限纳入 `series.control@1.0` 统一接管字段。
...
- 任务归属判定：纠正（我是说/改成/不对）、补充（还有/另外/对了）、媒体延续、前导语/未说完短句按同一任务；超过新任务间隔且无连续信号的独立消息不再硬合并，旧回复正常发出。
- 工具循环优先交给 AstrBot 原生 follow-up，在下一个 tool result 注入，不打断已有工具执行；普通单步生成走 stop + append 合并。
- 提交缓冲：只对"像没说完"的消息（在吗、逗号结尾、很短裸句）在发送前短暂等待补充；完整单句零额外等待，解决单条消息被固定防抖拖慢的问题。
- steering 模式下 rewrite 合并策略自动降级为 append，省掉一次 LLM 重写请求，保留用户原话。
- 群聊 sender / mention_or_sender 继续使用旧的固定窗口逻辑；私聊与 room 使用 steering。

## 0.8.16 - 2026-09-12

- 修复短回复被吞段：本地规则已能切出多段时，LLM 切分助手不得把结果压回单段（辅助仍可切得更细或并为至少两段）。
- 分段引导细化：只有单句极简回应才不分段；问候后又关心、连续问句、先回应再补充等多句回复按句分成多条发送。
- 切分助手提示词增加句界硬判据：句末标点（。！？）后还有完整句子时应切开，只有不可拆分的单句才允许返回单段。

## 0.8.15 - 2026-09-11

- 时间标注拟人化：连发结论句不再附带精确到秒的起止时刻；隔夜标签改用「凌晨/早上/上午/中午/下午/晚上/深夜」时段词替代 HH:MM（如「昨天 晚上」「09-05 下午」），模型看到的是人的时间语言。
- 插话合并与思考中断模板新增表达纪律：时间标注用于理解节奏而非复述，提到时间用「刚才」「几分钟前」这类自然说法。

## 0.8.14 - 2026-09-11

- 注入内容时间标注（规范 3.7「注入即标注」）：插话合并、思考中断合并、私聊最近承接、动态话题续接与群聊背景记录，逐条携带距现在的真实时间标注（刚刚/N秒前/N分钟前/N小时前，超过一天给出绝对时间）；"隔了多久"由代码换算，不再交给模型对裸时间戳心算。
- 秒级连发（首末条不超过 10 秒）在合并提示前追加结论句，明确"这 N 条是同一时刻的连续表达，不是隔了很久"，修复用户同一时刻连发多条私聊被当成跨时段消息逐条回应的问题。
- 插话追踪为每条用户消息记录平行到达时间，多次插话继承旧文本时连同各自原始时间一起继承；合并提示要点新增"按真实发送时间理解间隔，不要自行假设更多时间流逝"。
- 时间标注作为结构元数据不占用动态续接的内容字符预算；新增 `time_annotated_injections` 运行计数（`/convflow status` 可见），计数 fail-safe 不影响注入主路径。

## 0.8.13 - 2026-09-01

- 沉默判断指令按信任白名单分级注入：信任版模板与白名单 prejudge 跳过。
- 具身（伴夏/Quest）会话跳过 QQ 导向的分段与引用开销。

### 修复

- 修复 series control 接管、补丁与重置后子模块仍持旧配置的问题：重建配置后同步刷新沉默判断、分段、拦截判断与会话追踪等子模块，managed 覆盖即时生效。

## 0.8.12 - 2026-08-21

- 接入 `series.control@1.0`，支持言的分段与会话运行策略统一接管。

### 修复

- 群聊上下文注入新增当前发言者锚点（昵称 + 短匿名标记），明确历史 `@`、引用对象和上一轮回应对象不自动成为本轮回复对象，降低群聊多人连续发言时的称呼串线。
- LLM Provider 解析增加可选 `series.model_router@1.0` 核路由层；言自身配置优先，核不可用时透明回退原 AstrBot 链路。

## 0.8.11 - 2026-08-12

- 引用回复新增 `off / probability / llm_decides` 三态模式；`llm_decides` 由当前主模型在同一次请求内判断是否需要引用，不增加独立 LLM 调用。末尾内部标记在分段、TTS、历史和发送前严格清理，引用目标仍由言从真实事件解析，旧启用配置自动迁移为主模型模式。
- 修复私聊启用概率引用后随机出现突兀引用卡片的问题：新增独立私聊引用开关并默认关闭，旧配置即使保留总开关也只影响群聊；需要私聊引用时可显式开启。

## 0.8.10 - 2026-08-08

- 新增可选的按概率引用回复：在言的配置界面开启后，普通回复会按设定概率引用当前要回应的用户消息；默认关闭，分段只在首段引用一次，缺少公开消息 ID 或 Reply 组件时自动退回普通发送。

## 0.8.9 - 2026-08-08

- 调整“先发正文、随后单独 @Bot”的反向唤醒：上一条正文只恢复为本轮判断对象，主模型同时读取 `group_context_max_messages` 控制的最近群聊记录（默认 10 条），再结合发言关系、引用和 @ 指向决定自然回应或沉默；单独 @ 不再等同于强制回答。
- 收紧默认沉默判断边界：普通问候、直接称呼、呼唤、撒娇、求关注和表达亲近明确视为互动邀请；“妈妈”“姐姐”“在吗”等短消息不再因为缺少问题或正文而被误判成无需回应。
- `inject` 与 `prejudge` 统一采用上下文高门槛：`好的`、`嗯`、`谢谢`、`晚安` 等词不能脱离上下文直接判为话题终结；不确定时优先简短回应，言语攻击或不当请求交给安全规则正常处理而不是一律消失。

## 0.8.8 - 2026-08-07

- 调整智能分段优先级：开启 `chunking_llm_assist` 后，达到最小长度且主模型未用双空行明确分段的普通回复，优先交给 LLM 判断保持一条或按语义切成多条，不再只在本地候选段数超限时调用。
- 辅助 LLM 缺失、关闭、调用异常、超过 6 秒、返回非法 JSON 或改写/遗漏原文时，自动回退本地分段；只接受可逐字映射回原文的切分边界。
- 新增同会话动态话题续接：只在私聊公开历史缺页时，按轮数与文字预算补回较早真实轮次，由当前主模型判断是否继续原话题；完整历史、群聊和短承接已有兜底时不重复注入，也不增加独立 LLM 请求。
- 适配 `astrbot_plugin_stealer` 的真实 `&&emotion&&` 协议：仅在 stealer 已明确授权本轮自动表情时让出结果发送权，避免言提前分段阻断其 priority=100 的标签清理与发图；语音计划和短期历史只保留去标签正文。
- 兼容整条 `[silence]` 与 SpectreCore 的 `<NO_RESPONSE>` 沉默标记；正文引用或标记后仍有实际回答时不误判。
- 修复诊断日志清空后已生成新 `stream_id` 却沿用旧序号的问题；新日志流现在从 1 重新计数，轮询方仍通过 `stream_id` 安全识别换流。

## 0.8.7 - 2026-08-07

### 新增

- 新增版本化的 `conversation.proactive_message@1.0` 通用主动文本交付契约：调用方只需提交已经生成好的私聊文本和自然人身份，言负责授权复核、复用现有 chunker/延迟分段与安全发送，不增加 LLM 调用；环境主动消息同步改走共享交付函数。

### 修复

- 修复默认沉默标记被模型输出为 `<SILENT/>`、`<SILENCE>`、`<SILENCE />` 或 HTML 转义形式时泄漏的问题；仅在高置信开头标签和短误加解释条件下兼容变体，不把正文引用、普通单词或任意 `&&...&&` 文本误判为沉默。
- 收紧正式及自定义沉默 marker：仅接受开头控制语境和空/严格短解释，正文引用或带明显回答内容的 marker 不再误触发。

### 测试

- 增加契约发现、结构化请求、私聊/权限失败关闭、双换行分段、分段延迟、首段发送失败单次完整文本回退、部分发送不重复，以及旧环境主动交付兼容测试。
- 增加精确 marker、默认标签变体、短解释、正文引用、自定义 marker、HTML 转义和关闭沉默注入时的兼容回归测试。
...
- 新增可选的关系冒犯候选链：开启后由主模型一次判断直接针对 Bot 的恶意辱骂、骚扰或威胁，严格标记经 relationship.event@1.0 脱敏提交给“情”；默认关闭，不增加额外模型调用。

### 安全

- 仅接受完整的置信度和严重度属性，并复核原始 bot、用户、群聊与平台作用域；缺失契约、身份歧义、低置信度和提交异常均失败关闭。
- 内部标记无论格式是否有效都会在文本分段和语音规划前清理；用户、历史、引用或工具返回中的同名标记不能直接触发关系记账。

### 测试

- 增加默认关闭、白名单跳过、严格解析、平台作用域、幂等提交、格式错误只清理不记账，以及文本与语音标记防泄漏回归测试。
## 0.8.4 - 2026-08-03

### 变更

- 将配置页说明统一改为中文大白话，明确实验性防抖、打断、图片与跨会话上下文等设置的影响和风险。
- 配置 schema 的数值边界与运行时钳制范围保持一致，减少页面能保存但运行时被悄悄修正的落差。

### 测试

- 增加 schema 边界回归测试，覆盖最小值、最大值和运行时配置归一化行为。

## 0.8.3 - 2026-08-03

### 修复

- 修复 Agent 工具调用前的空白/空文本 LLM 中间装饰帧提前 `finish_response`、导致最终分段正文无法写入 `CompletedTurn` 的问题。言现在利用 AstrBot 4.26.8 在 AgentRunner 前只触发一次 `on_llm_request`、仅在 Agent 真正终态触发 `on_llm_response` 的生命周期事实管理 event 终态标志：新 Agent 请求先重置为 `false`，真正终态再设为 `true`；中间空白帧保留 pending，只有终态空白帧才收敛 tracker，同一 event 复用时也不会继承上一轮终态。
- 最终非空分段正文继续按实际发送内容记录为最近完成轮次，下一条私聊短评会承接新正文，不再退回更旧的话题。

### 测试

- 增加“工具前空白中间帧 -> 最终分段正文 -> 短回复承接新正文”端到端回归测试、真正终态空白帧清理且不生成虚假完成轮次的测试，以及同一 event 开启新 Agent 请求时终态标志重置的生命周期回归测试。

## 0.8.2 - 2026-08-02

### 修复

- 修复 AstrBot 4.26.8 原生 follow-up 与言重复捕获同一插话的问题：言在核心 follow-up 捕获前以最高优先级接管符合时间窗的私聊/唤醒插话，使用公开 Agent 停止能力；群聊 sender 作用域仅在可用的窄兼容入口中停止同一发送者，入口不可用时安全降级。
- 恢复插话的基础语义：新消息到达时尚未发送的旧结果作废，旧文本与真实图片、音频及有效图片描述会按旧消息在前的顺序合并到下一次主模型请求；不再把媒体降级为单纯的“[图片]”占位符，也不新增额外 LLM 预判调用。
- 收窄智能分段兜底：LLM 生成的双空行仍是权威分段结果；短于 chunking_min_length 的文本只有在存在全角/半角强语气句界、前后两句均达到自然长度且后句完整时，才放宽一次拆分，避免“好呀！嗯。”等碎片化回复。逗号、普通句号、省略号和代码块行为保持不变。

### 测试

- 增加 AstrBot 4.26.8 follow-up 早期接管、思考阶段默认合并、图片/音频/图片描述继承及多次插话媒体顺序回归测试。
- 增加中文/英文感叹号、连续语气标点、短句防碎片及双空行优先的分段回归测试。
...
- 修复私聊短答被长记忆或陪伴状态覆盖的问题：同一次主模型请求内保留并行提示注入，在 Memory Companion 与情的后置只读桥完成后，仅将现有“紧邻上一问 + 当前消息”承接块去重并移到末尾；明确保护肯定、否定和纠正方向，不新增 LLM 调用。

### 测试

- 增加短否定“没呢”、承接块后置去重及系列 `-40` 钩子顺序回归测试。

## 0.8.0 - 2026-07-31

### 头像

- 更新凝心溯溪-言 的插件头像为本次发布的 `logo.png`。
...
- 新增同一自然人的跨会话近期弱感知：只驻留内存的有界事件池按当前会话优先、明确跨窗口指代、话题重合和分层时间资格选择最多两个证据胶囊，不联网、不新增 LLM 调用。
- 接入情的 `relationship.continuity_identity@1.0` 匿名等价身份和序的 `identity.context_bridge_authorization@1.0` 逐轮读取授权；新增五项大白话配置、`/convflow status` 缓存/命中统计及统一请求上下文去重标志。

### Security

- 事件池不保存原始 UID、平台、UMO、系统提示、工具参数/结果、纯媒体占位或第三方群成员消息；来源、事件和连续身份均使用进程随机密钥生成的不透明摘要，重载或重启清空。
- 私聊进入群聊默认拒绝。`topic_only` 只确认相关私聊存在，且只允许当前群消息已经公开的词；`details` 必须由当前群消息明确授权、限制为必要短片段并遮盖常见凭据。否定、假设、引用伪造、契约缺失或身份歧义全部失败关闭，群聊之间不搬运。
- 私聊进入群聊即使明确授权也最多选一个最近相关私聊来源，不会把多个私聊窗口合并公开；事件正文、活动数量和内部淘汰索引均保持有界。
- 当前会话存在相关轮次时禁止其他会话抢占；低信息消息仅在当前会话无焦点且五分钟内恰好一个其他私聊候选时承接，多个候选不猜。时间不能单独获得注入资格。

### Changed

- 言选中近期跨会话胶囊后发布 `flags.conversation_flow.recent_context_selected=true`，情据此跳过本轮其他账号 Memory Companion 长期桥；未选中时长期召回保持原行为。
- 只有近期弱背景确实写入模型请求后才发布去重标志；注入失败时保留情的长期记忆桥，并在请求诊断中记录稳定原因码。业务健康诊断现同时检查近期活动缓存是否就绪。

### Tests

- 新增事件清洗、容量/TTL、幂等、淘汰索引上限、当前会话优先、低信息歧义、私聊/群聊方向、群聊明示同意两档、单一私聊公开来源、私密锚点防泄漏、凭据遮盖、契约接线、实际回复记录、配置范围、注入失败及长期记忆去重回归测试。

## 0.7.3 - 2026-07-30

### Fixed

- 修复 AstrBot 4.26.7 将图片描述写入 `req.extra_user_content_parts` 并清空 `req.image_urls` 后，言误判主模型看不到图片、跳过图片意图提示的问题。现支持 `TextPart.text` 与文本字典中的有效 `<image_caption>` 和既有视觉摘要标记，同时排除描述失败、附件路径、文件路径与单纯图片占位符；诊断日志只记录来源，不输出描述正文。
- 图片意图提示改为分别判断媒体形态与对话作用：表情包不再默认等同于话题结束，方形或动漫人物特写不再被凭空认作头像；沉默收口必须同时满足任务已完成、无待处理内容、明确结束交流且没有互动意图，不确定时优先自然回应。

### Tests

- 新增 `req.image_urls`、`TextPart`/字典图片描述、额外视觉摘要、描述失败、附件路径、文件路径、图片占位符、单次主模型注入及表情包/头像/不确定场景提示词边界回归测试。

## 0.7.2 - 2026-07-30

### Added

- 新增 `conversation.proactive_delivery@1.0`：为境的后台环境候选提供普通回复上下文准备与真正主动私聊交付入口。
- 主动环境消息由对话模型结合结构化环境事实与情的派生语气建议决定是否发送和最终措辞；普通回复仍由原主模型一次判断，不增加预分类 LLM。

### Security

- 每次候选注入或主动发送前都重新校验序的 `identity.proactive_authorization@1` 与情的 `relationship.delivery_identity@1`；旧候选、群聊目标、未授权会话、账号归属不符、静默建议、缺失或不兼容契约均失败关闭，不使用模板直发。
- 环境候选按不可信数据处理，只读取版本化结构字段，不执行其中指令；最终主动文本限制长度并禁止暴露内部机制。
- 候选只接受已知环境事件与对应事实字段，地点和文本字段均限制长度；模型异常或返回格式错误时不发送。最终文本在发送前移除格式、限制为 120 字，并实际拦截内部机制词和服务式追问，而不只依赖提示词约束。

### Tests

- 新增依赖缺失失败关闭、旧候选拒绝、未知事实字段、结构化候选背景、模型异常、机制泄漏、服务式追问、长度上限、模型抑制及成功私聊发送回归测试。

## 0.7.1 - 2026-07-29

### Fixed

- 修复群聊中“先发正文、随后单独 @Bot”进入 AstrBot 空 @ 等待、导致上一条问题未被处理的问题。言会在可配置的短时间窗内恢复同一发送者尚未消费、未曾唤醒 bot 的上一条纯文本，再交给正常 LLM 链路；不会跨群、跨用户、搬运不完整的复合消息或连续重复消费。

### Tests

- 新增反向唤醒的同用户、时间窗、单次消费、消息链恢复与配置边界测试。
