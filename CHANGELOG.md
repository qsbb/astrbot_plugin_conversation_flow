...
- 设置页删除与设置区重复的「当前配置摘要」卡片，移除冗余的 `GET /config` 请求及对应端点（配置值已随 schema 一并返回）；页面文案去掉「覆盖层」「核 WebUI 字段接管」等内部说法；引用回复、错字理解、安静合并相关注释统一为当前态陈述。
- 移除两个已失效的插话配置项 `steering_new_turn_gap_ms`、`steering_uncertain_gap_ms`（状态驱动判据已取代按间隔判定，二者无消费点；旧配置中的残留键会被安全忽略）；移除无生产调用的 `task_relation()` 规则函数及对应测试。
- 同步系列共享层 `series.ui` 1.0.3（按钮基线不换行；新增共享转义、响应解析、API 工厂、JSON 导出工具）。

## 0.12.11 - 2026-09-18

### 新增

- 群聊唤醒场景分级：区分「裸@叫人 / @+正文 / @+引用+正文」。裸@时引导她看一眼带编号的群记录挑值得接的话；@+引用时优先围绕被引用内容回应，默认引用目标自动切换为被引用的那条。
- 群聊上下文唤醒注入带 #n 编号与优先标记：「@你带正文」标（← 在叫你）、「回复你发的」标（← 回复你）。
- `reply_with_quote` 工具新增可选 `target` 参数：可引用群聊上下文记录里的任意一条（传 #n 编号），非法编号自动回落当前消息。
- 错字/同音字理解注入（`typo_interpretation_enabled`，默认开）：结合上下文理解错别字、谐音梗、缩写、口误，自然回应不刻意纠正；私聊群聊均生效。
- 打断后安静合并：新消息打断了一条还在思考的回复时，等一个安静窗口再合并重跑，避免连发消息反复打断；首条消息仍立即响应。新配置 `merge_settle_enabled`（默认开）/ `merge_settle_ms`（4000）/ `merge_settle_max_ms`（15000，自首条消息起的总封顶）。

### 变更

- 引用回复决策从文本标记迁移为 LLM 工具调用（`reply_with_quote`），正文不再出现内部标记；旧标记解析保留作不支持工具调用服务商的兜底。
## 0.12.10 - 2026-09-16
