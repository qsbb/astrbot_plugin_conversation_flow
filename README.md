...
| `reply_quote_enabled` | `false` | 开启后，Bot 在允许的会话范围内按概率引用当前要回复的用户消息；只在有消息 ID 且 Reply 组件可用时生效 |
| `reply_quote_private_enabled` | `false` | 是否允许私聊也使用引用回复；默认关闭，避免连续私聊中出现突兀的引用卡片 |
| `reply_quote_probability` | `30` | 每轮回复的引用概率，范围 0-100；分段回复只在第一段引用一次 |
...
开启“按概率引用回复”后，言会把当前收到、并准备回应的那条用户消息作为引用目标，而不是引用无关历史或模型自己的上一条回复；默认仅群聊参与抽样，私聊必须额外开启 `reply_quote_private_enabled`，从而避免 QQ 连续私聊出现没有必要的引用卡片。“先发正文、再单独 @Bot”时会引用前一条正文，不会引用只有 @ 的空消息。引用决定在本轮只抽取一次，发送多段时不会每段都带引用。平台没有有效 `message_id`、AstrBot 没有公开 `Reply` 组件，或组件构造失败时会保持原来的普通发送，不额外重试、不增加 LLM 调用。
