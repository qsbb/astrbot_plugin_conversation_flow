...
**智能分段优先级**（v0.3.0+）：
1. **LLM 双空行分段**（`\n\n`）视为强分段信号，每段保留不切，尊重 LLM 的主动分段意图；
2. 超长段落（> `chunking_long_paragraph_threshold`）即使有双空行也按句末标点继续切分；
3. 无双空行时整体按句末标点（`。！？!?…`）切分；
4. 短段落（≤ threshold）保留不切。
...
4. **超长段落才按句末标点切分**：段落 > `long_paragraph_threshold` 时按 `。！？!?…` 切分；无双空行的文本整体按句末标点切分；
