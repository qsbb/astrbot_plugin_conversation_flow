...
| `on_llm_response` | Agent 终态标记 + 沉默标记检测 + 拦截标记检测 | 区分工具调用中间帧与真正结束；主 LLM 已生成回复但应静默 |
| `on_decorating_result` | 组件感知分段 + 纯文本后处理 + 终态收敛 + 最终收尾计数 | 工具前空白中间帧误清理上下文；一大串文字糊在一条消息里；图文/音频混合链被整体跳过；Markdown 格式破坏聊天自然度；原始但未发送的回复污染行为计数 |
...
这些片段始终属于同一次主模型请求，不会因为插件分别注入而生成两次回复。私聊短答的承接块
先随主链生成；Memory Companion（`-20`）与情的跨平台只读桥（`-30`）完成后，言在 `-40`
只把同一个承接块去重并移到附加内容末尾，不新增片段或模型调用。承接块同时列出紧邻对话和
当前短答，并要求肯定、否定、纠正以当前消息为准，避免较长记忆或日程背景覆盖“没呢”这类明确回答。
...
**Agent 终态收敛**：AstrBot 4.26.8 的 `on_llm_request` 在 AgentRunner 启动前触发一次、不会在
工具循环内部重入；言会在这里先把当前 event 的终态标志重置为 `false`，避免 activated handler
或后续 ProviderRequest 复用 event 时继承上一轮终态。`on_llm_response` 只在 Agent 真正结束后
触发，届时再把终态标志设为 `true`。工具调用前出现的空白或空文本 LLM 装饰帧只记诊断、不结束
tracker；只有带终态标志的空白帧才清理 pending。最终非空正文仍按原有文本/组件分段路径交付，
并把实际发送的完整正文写入 `CompletedTurn`，供下一条私聊短评自然承接。
...
沉默采用高门槛：只有明确收口且没有待回答内容、用户明确要求停止交流，或内容确实是无法承接的纯噪声时才允许静默。普通问候、直接称呼、呼唤、撒娇、求关注和表达亲近属于互动邀请；“早上好”“在吗”“妈妈”“姐姐”“抱抱”“看看我”等消息即使很短也应自然回应。`好的`、`嗯`、`谢谢`、`晚安` 等词必须结合上下文判断，不能只凭词语本身认定话题结束；不确定时优先简短接话。
...
| `group_context_max_messages` | `10` | 获取的最近群聊消息条数上限；反向唤醒同样使用此数量，可修改 |
| `group_context_only_when_woken` | `true` | 仅在被唤醒时注入；关闭则每次群聊消息都注入（消耗更多 Token） |
| `group_context_reverse_wake_enabled` | `true` | 支持同一用户先发正文、随后单独 @Bot，并由主模型结合上下文判断是否回应 |
...
当用户采用“`吃什么好呢` → 单独 `@Bot`”的发送顺序时，AstrBot 原生空 @ 流程只会等待 @ 之后的下一条消息，不会向前取正文。反向唤醒会在内置等待器启动前，把同一群、同一用户、短时间内尚未消费且未曾唤醒 bot 的上一条纯文本恢复为当前判断对象，并按 `group_context_max_messages`（默认 10）提供最近群聊记录。单独 @ 只表示让 bot 回看并判断是否适合参与，不等于上一句话一定在问 bot，也不等于必须回复；主模型会结合前后发言、引用和 @ 指向决定自然接话或沉默。带图片、引用或其他 @ 的消息不会被不完整地搬运，连续空 @ 也不会重复处理同一条正文，超时后仍保持 AstrBot 原生行为。
...
每次群聊上下文注入还会附加一个当前发言者锚点：只提供昵称和按群聊/账号计算的短匿名标记，不暴露原始 UID。模型默认只回应本轮发送者；历史里的 `@`、引用对象或上一轮回应对象不会自动成为当前对象，除非当前消息明确要求讨论或称呼对方。
...
| `reply_quote_mode` | `off` | 引用决策：`off` 关闭、`probability` 本地概率、`llm_decides` 由当前主模型同链判断 |
| `reply_quote_private_enabled` | `false` | 仅限制 `probability` 模式是否允许私聊随机引用；主模型模式会自行判断私聊是否真有必要引用 |
| `reply_quote_probability` | `30` | `probability` 模式的引用概率，范围 0-100；分段回复只在第一段引用一次 |
...
`llm_decides` 不新增一次模型请求：言只在当前主模型请求中追加引用判断规则；模型认为引用能明显减少歧义时，在完整回复末尾输出内部 `<REPLY_QUOTE/>` 标记。言会在分段、TTS、历史记录和实际发送前严格剥离该标记，再用事件中的真实消息 ID 构造 `Reply`；模型不能指定引用 ID。普通连续私聊、问候、确认、短问答和情绪互动默认不引用，群聊对象不清或精确纠正原话时才倾向引用。正文讨论标记、标记不在独立末尾、没有有效消息 ID、请求已被插话作废或本轮未注入判断规则时均失败关闭。

`probability` 保留原来的纯本地随机模式；默认仅群聊参与抽样，私聊必须额外开启 `reply_quote_private_enabled`。旧版本中 `reply_quote_enabled=true` 的配置升级后自动迁移到 `llm_decides`，不再继续随机引用。“先发正文、再单独 @Bot”时仍引用前一条正文，不会引用只有 @ 的空消息。两种模式都只在本轮第一段添加一次引用；平台没有有效 `message_id`、AstrBot 没有公开 `Reply` 组件或组件构造失败时保持普通发送。
...
[对话流控制指令 - 沉默判断]
只有在高把握确认无需回应时才允许沉默：
1. 已完成事项明确收口，且没有待回答内容、情绪或互动信号；
2. 确实无法理解和承接的纯噪声；
3. 用户明确要求停止交流。

普通问候、称呼、呼唤、撒娇、求关注和表达亲近必须回应；短句不等于话题结束。不确定时优先简短接话。
只有确认属于以上情况时输出 <SILENCE/>；否则正常回复。
```
...
3. **`event.send()` 与空日志**：分段发送依赖适配器实现，极少数适配器可能不支持在 `stop_event()` 后主动发送。AstrBot 4.26.7 可能对同一已停止、已清空的事件记录两次空 `Prepare to send`，并把既有停止状态归因到首个发送后钩子；这不是第二次 LLM 请求，也不会再发送一份可见回复。
