...
| 字段 | 默认 | 说明 |
|---|---|---|
| `image_intent_mode` | `false` | 检测到用户发送图片时，注入指令让主 LLM 判断图片类型并决定回复方向 |

判断三类图片并决定回复方向：
1. **无意义表情包/贴图**（话题终结、刷屏）→ 输出 `<SILENCE/>` 沉默收口
2. **表达观点/态度的表情包**（赞同、反对、吐槽）→ 结合态度自然回应
3. **包含信息的图片**（截图、照片、文档）→ 根据图片内容正常回复
