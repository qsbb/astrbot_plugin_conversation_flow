"""拟人化情绪：让 bot 被反复打扰时可以懒得回、甚至装作没看见。
...
class MoodTracker:
    ...
    def _calculate(self, state: _MoodState, normalized_text: str) -> tuple[int, int, list[str]]:
        interaction_count = len(state.interactions)
        repeat_count = 0
        if normalized_text:
            repeat_count = sum(1 for _ts, value in state.texts if value == normalized_text)
    ...
    def evaluate(self, scope_key: str, user_text: str, now: float | None = None) -> MoodDecision:
        """记录本轮互动并返回情绪判定。"""
        if not scope_key:
            return MoodDecision()
    ...
    def cleanup_stale(self, ttl_seconds: float | None = None, now: float | None = None) -> int:
        """清理长期无活动的会话，返回数量。"""
        current = time.time() if now is None else float(now)
