...
def test_series_diagnostics_are_bounded_redacted_and_do_not_propagate():
    diagnostic_clear()
    diagnostic_event(
        "flow.bridge",
        "bridge ready for 123456789",
        details={"count": 2, "user_id": "123456789"},
    )
    logger.warning(
    ...
    assert payload["events"][0]["details"] == {"count": 2}
...
def test_series_diagnostics_cursor_and_capacity():
    ...
    for index in range(1005):
        diagnostic_event("flow.event", index)
